...
(function (global) {
  ...
  const IDS = (global.ChestIds && global.ChestIds.steps) || {};
  ...
  function calculateHeartLikeScore(addFeature, bumpDx) {
    ...
    let historyPoints = 0;
    const site        = getOptionLabel(IDS.site);
    const character   = getOptionLabel(IDS.character);
    const aggravating = getOptionLabel(IDS.aggravating);
    const relief      = getOptionLabel(IDS.relief);
    const radiation   = getOptionLabel(IDS.radiation);
    ...
    let riskPoints = 0;
    const pmhLabel = getMultiLabels(IDS.pmhChronic).join("، ");
    const famLabel = getOptionLabel(IDS.familyHistory);
    const shLabel  = getMultiLabels(IDS.socialHistory).join("، ");
  ...
  function calculatePEHeuristicScore(addFeature, bumpDx) {
    ...
    const onsetLabel      = getOptionLabel(IDS.onset);
    const characterLabel  = getOptionLabel(IDS.character);
    const associatedLabel = getMultiLabels(IDS.associated).join("، ");
    const rosLMLabel      = getMultiLabels(IDS.rosLM).join("، ");
    const pshOpsLabel     = getMultiLabels(IDS.pshOps).join("، ");
    const pmhLabel        = getMultiLabels(IDS.pmhChronic).join("، ");
  ...
  function applyTextHeuristics(addFeature, bumpDx) {
    ...
    const durText = normaliseText(ans[IDS.ccDuration] || "");
